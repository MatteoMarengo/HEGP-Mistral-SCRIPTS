...
#######################################################################################################
app.layout = html.Div([
dcc.Markdown('# Prostate Radiotherapy Patient Symptom Intake Form - Version Mistral7B', style={'text-align': 'center'}),
    html.P([html.Br()]),
...
#######################################################################################################
def update_table_results_gpt4(n_clicks, *responses):
    if n_clicks == 0:
        return None, []
    ...
    summary = summarize_table_gpt4(data,language)
    return summary, data
